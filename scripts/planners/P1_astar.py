...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        free = self.occupancy.is_free(x)
        return x[0] >= lo[0] and x[1] >= lo[1] and x[0] <= hi[0] and x[1] <= hi[1] and free
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        ########## Code starts here ##########
        d = self.resolution
        pn1 = self.snap_to_grid((x[0]-d, x[1]-d))
        pn2 = self.snap_to_grid((x[0]-d, x[1]))
        pn3 = self.snap_to_grid((x[0]-d, x[1]+d))
        pn4 = self.snap_to_grid((x[0], x[1]+d))
        pn5 = self.snap_to_grid((x[0]+d, x[1]+d))
        pn6 = self.snap_to_grid((x[0]+d, x[1]))
        pn7 = self.snap_to_grid((x[0]+d, x[1]-d))
        pn8 = self.snap_to_grid((x[0], x[1]-d))
        p_neighbors = [pn1, pn2, pn3, pn4, pn5, pn6, pn7, pn8]
    ...
    def solve(self):
        ...
        while self.open_set:
            ...
            for q_prime in self.get_neighbors(q):
                if q_prime not in self.closed_set:
                    q_prime_cost = self.cost_to_arrive[q] + self.distance(q, q_prime)
                    if q_prime_cost < self.cost_to_arrive[q_prime]:
                        self.came_from[q_prime] = q
                        self.cost_to_arrive[q_prime] = q_prime_cost
                        self.est_cost_through[q_prime] = q_prime_cost + self.distance(q_prime, self.x_goal)
                        self.open_set.add(q_prime)
        return False
        ########## Code ends here ##########
