...
class TrajectoryTracker:
    ...
    def compute_control(self, x: float, y: float, th: float, t: float) -> T.Tuple[float, float]:
        ...
        u1 = xdd_d + self.kpx*(x_err) + self.kdx*(xd_err)
        u2 = ydd_d + self.kpy*(y_err) + self.kdy*(yd_err)
        V = self.V_prev + (u1*np.cos(th)+u2*np.sin(th))*dt
        
        if V < V_PREV_THRES:
            V = np.sqrt(np.square(xd)+np.square(yd))
        
        om = (u2*np.cos(th)-u1*np.sin(th))/self.V_prev
